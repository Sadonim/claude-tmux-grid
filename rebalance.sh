...
# Column widths + row heights: resize the top pane of each column for both axes.
# In tmux each column is an independent vertical split, so every column's
# first-row pane must be resized individually to equalise heights across columns.
