...
build_layout_n() {
  ...
  # ── Bottom grid (ROW-FIRST: all panes in a row share the same horizontal
  #    divider, eliminating misaligned row heights from independent column splits)
  if [ "$bottom_n" -gt 0 ]; then
    # 1. Split bottom area into rows first
    local row_origins
    split_into_n -v "$bottom_origin" "$bottom_rows" row_origins

    # 2. Split each row into columns; accumulate pane IDs row-major
    # all_row_panes[row * bottom_cols + col] = pane_id
    local all_row_panes=()
    local r=0
    while [ "$r" -lt "$bottom_rows" ]; do
      local col_panes_r
      split_into_n -h "${row_origins[$r]}" "$bottom_cols" col_panes_r
      local c=0
      while [ "$c" -lt "$bottom_cols" ]; do
        all_row_panes+=("${col_panes_r[$c]}")
        c=$(( c + 1 ))
      done
      r=$(( r + 1 ))
    done
    ...
    # all_row_panes[row * bottom_cols + col]
    ...
    while [ "$row" -lt "$bottom_rows" ]; do
      ...
      while [ "$c" -lt "$bottom_cols" ]; do
        ...
        if [ "$agent_global" -lt "${#AGENTS[@]}" ]; then
          local pane_idx=$(( row * bottom_cols + c ))
          register_agent "${all_row_panes[$pane_idx]}" "${AGENTS[$agent_global]}"
        fi
...
build_layout_agents_only() {
  ...
  # Row-first construction: split into rows, then each row into columns
  local row_origins_ao
  split_into_n -v "$MONITOR_PANE" "$rows" row_origins_ao

  local all_panes=()
  local r=0
  while [ "$r" -lt "$rows" ]; do
    local col_panes_r
    split_into_n -h "${row_origins_ao[$r]}" "$cols" col_panes_r
    local c=0
    while [ "$c" -lt "$cols" ]; do
      all_panes+=("${col_panes_r[$c]}")
      c=$(( c + 1 ))
    done
    r=$(( r + 1 ))
  done
...
setup_borders() {
  ...
  # Colour is derived from the status icon in pane_title (set by update-borders.sh):
  #   !  orange  approval needed (user action required)
  #   ●  cyan    thinking / running
  #   ✓  green   done
  #   ✗  red     error
  #   ○  grey    idle (default)
  local ICON_FMT
  ICON_FMT='#{?#{m:! *,#{pane_title}},#[fg=colour214],'
  ICON_FMT+='#{?#{m:● *,#{pane_title}},#[fg=colour51],'
  ICON_FMT+='#{?#{m:✓ *,#{pane_title}},#[fg=colour82],'
  ICON_FMT+='#{?#{m:✗ *,#{pane_title}},#[fg=colour196],'
  ICON_FMT+='#[fg=colour238]}}}} #{pane_title} #[default]'
  tmux set-option -w -t "$SESSION:$WINDOW" pane-border-format " $ICON_FMT"
